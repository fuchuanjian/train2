...
public class StateGame extends GameState {
	...
	private void trainFinished(int color) {
		...
		if (!flag) {
			...
			this.success = true;
			int curLevel = super.game.getValue(EValues.EValueSelectedLevel);
			if ((super.game.getSettings().m_levels.get(curLevel - 1) < 1)
					&& (((curLevel == 5) || (curLevel == 15)) || (((curLevel == 20) || (curLevel == 30)) || (curLevel == 0x2d)))) {
				this.shouldDoLevelAnim = curLevel - 1;
				super.game.setValue(EValues.EValueDoLevelSelectAnimation,
						this.shouldDoLevelAnim);
			} else if (((curLevel == 15) || (curLevel == 30)) || (curLevel == 0x2d)) {
...
//					int curLevel = game.getValue(EValues.EValueSelectedLevel);
//					game.getSettings().saveSuccessLevel(curLevel-1);
				}
	...
	public void paint(Painter painter) {
		if (this.tutorialId != -1) {
			...
			//画顶部的山体
			int leftTileHalfX = this.levelx + (this.tilew / 2);
			int topTileHalfY = this.levely - (this.tileh / 4);
			int tielhUnit = this.tileh / 10;
			for (int i = 0; i < this.LEVEL_TILES_COLS; i++) {
				int num30 = (topTileHalfY + ((GameUtils.sin((this.gametick * 10)
						- (i * 60)) * tielhUnit) >> 14))
						+ (tielhUnit / 2);
				if (this.levelSet == 2) {
					this.mountaintop.PaintScaled(painter, (float) leftTileHalfX,
							(float) num30, 0, 0.7f, 0.7f);
				} else {
					this.mountaintop.Paint(painter, (float) leftTileHalfX,
							(float) num30, 0);
				}
				leftTileHalfX += this.tilew;
			}
			//画边缘的山
			int num31 = this.levelx - ((this.tilew * 5) / 22);
			...
			int cavesSize = this.caves.size();
			int index = 0;
			int num45 = 0;
			if (this.trains.size() > 0) {
				for (int i = 0; i < this.trains.size(); i++) {
					int y = this.trains.get(i).y;
					if (index < cavesSize) {
						boolean flag2 = true;
						while (flag2) {
					...
					if (this.trains.get(i).getEntityType() == EEntityClass.EEntityTrain) {
						...
						int type = this.trains.get(i).type;
						int x = this.trains.get(i).x;
						int num50 = this.trains.get(i).y;
						if ((type >= 0) && (type <= 2)) {
							num50 += this.tileh / 2;
							float angle = 0f;
			...
			for (int num53 = 0; num53 < cavesSize; num53++) {
				...
				if (this.success) {
					...
					if (this.btnplay.paint(painter, super.game, num121, num117)
						...
						if (this.shouldDoLevelAnim != -1) {
							...
							int curLevel = super.game
									.getValue(EValues.EValueSelectedLevel);
							if (super.game.isTrial() && (curLevel == 5)) {
